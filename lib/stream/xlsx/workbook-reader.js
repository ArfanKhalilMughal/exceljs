...
class WorkbookReader extends EventEmitter {
  ...
  async *parse(input, options) {
    ...
    // Collect all entries into temp files first (fully consuming the ZIP),
    // then process them in-order so sharedStrings/styles/rels are available.
    const allEntries = [];
    await new Promise((resolve, reject) => {
      zip.on('entry', entry => {
        allEntries.push({path: entry.path, entry});
        // Must consume each entry immediately so unzipper advances.
        // We collect a buffer for small entries and use a temp file for sheets.
        const chunks = [];
        entry.on('data', chunk => chunks.push(chunk));
        entry.on('end', () => {
          allEntries[allEntries.length - 1].buffer = Buffer.concat(chunks);
        });
